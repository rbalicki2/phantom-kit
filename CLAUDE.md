...
- **Multiple conditions require array wrapping.** Use `[:key :action [["dsk_layer" 0] ["dsk_return_to_layer" 1]]]` - the conditions must be wrapped in an outer array. Without the array, only the first condition is used. Block-level conditions (`:!apple_internal :Chrome`) also combine with per-rule conditions.
...
- **`{:alone ...}` requires explicit `nil` if no condition.** Goku rules are `[from to condition options]`. If `{:alone ...}` is in position 3 without a condition, Goku misparses it. Use `[from to nil {:alone ...}]` to explicitly indicate no condition. Example: `[{:key :right_control} [:right_control] nil {:alone [:vk_none ...]}]`.
...
**Key principle**: The grid tool only handles mouse POSITIONING. We handle all clicks ourselves via Hammerspoon. This gives us full control over click modifiers.

**Keys:**
- Navigation: u,i,o,j,k,l,m,comma,period (3x3 grid positions)
