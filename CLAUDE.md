...
- `init.lua` - Hammerspoon config for layer overlay (Ctrl+Shift+Comma)
...
**Overlay files (`layers/*.txt`)**: These are shown to the user via Ctrl+Shift+Comma overlay. If you change shortcuts in a layer, you MUST update the corresponding `.txt` file or the overlay will be wrong.
...
- **Ctrl+Shift+Comma** shows overlay with current layer's shortcuts
