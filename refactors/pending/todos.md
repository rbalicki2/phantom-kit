...
- AltIns `]` key as held shift: Tap `]` = Function Shift (oneshot shift submode, same as current Fn+]). Hold `]` = continuous shift (active for as long as key is held, like a physical shift key). Move forward delete to Shift+Backspace instead. Currently: `]` → Delete (R2065), Fn+] → shift-oneshot (R2080).
