...
- Double-tap N for Nav layer from Insert mode: Currently Ctrl+Escape (physical N on thumb cluster) exits to Normal. Want: single N in regular Insert mode types normally, double-N enters Nav layer. In AltIns mode the physical key is H (h/n mapping). This would provide quick Nav access without leaving Insert flow. Needs double-tap detection pattern (similar to comma→period double-tap in AltIns submode 10).
