...
- [DONE] AltIns `]` key as held shift+fn: Tap `]` = oneshot shift submode (R2065 alone→submode 1). Hold `]` = right_option modifier (R2065 held→right_option), with R3193-R3215 mapping right_option+key → shift+fn output for each key. Forward delete moved to Shift+Backspace (R3216).
