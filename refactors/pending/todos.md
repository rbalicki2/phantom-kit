...
- AltIns `]` key behavior: Bare `]` tap should be oneshot shift (Cmd+Shift), but when `]` is held down it should act as Fn+Shift (continuous shift active for the duration of the hold). Currently: `]` → Delete (R2065), Fn+] → shift-oneshot (R2080). Move forward delete to Shift+Backspace instead.
