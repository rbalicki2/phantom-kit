...
- AltIns `]` key as held shift+fn: Tap `]` = Function Shift (oneshot shift submode, same as current Fn+]). Hold `]` = continuous shift+fn (each key outputs its shift+fn column value from rhs-slots.md for the duration of the hold). Needs its own submode because shift+fn outputs are custom per-key mappings. Implementation: use Karabiner held/alone pattern — `alone` enters oneshot shift submode, `held` sets a new submode value with per-key rules for shift+fn outputs, `to_after_key_up` clears submode back to 0. Move forward delete to Shift+Backspace. Currently: `]` → Delete (R2065), Fn+] → shift-oneshot (R2080).
- Inline gh-pr-create-base.sh: The Fn+U `gh pr create --base <auto>` rule (R3191) calls an external shell script (`scripts/actions/gh-pr-create-base.sh`). This won't work on other machines where the repo isn't at the same path. Inline the logic into the Karabiner rule (e.g., a single shell_command string) so the config is self-contained.
- Nav mode N → Escape: N in Nav mode currently opens Chrome Personal (R0206). Change it to send Escape instead (Chrome Personal would need a new home). N from Normal already enters Nav, so N→Nav→N→Escape would be a natural "double-N for Escape" flow. Also: HK4 (Alt+F20) is now free in InApp mode — available for a new shortcut.
